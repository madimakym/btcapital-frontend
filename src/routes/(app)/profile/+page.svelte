    ...
    try {
      if (user?.role === 'INVESTOR') {
        await profileApi.updateInvestorProfile({
          bio, country, city, linkedinUrl, websiteUrl,
          investmentRangeMin, investmentRangeMax, preferredSectors, investmentStages
        });
      } else if (user?.role === 'ENTREPRENEUR') {
        await profileApi.updateEntrepreneurProfile({
          bio, country, city, linkedinUrl, websiteUrl,
          companyName, companyDescription, sector, stage, fundingNeeded
        });
      } else if (user?.role === 'INCUBATOR') {
        await profileApi.updateIncubatorProfile({
          bio, country, city, linkedinUrl, websiteUrl,
          incubatorName, description, focusSectors, programDuration, maxStartups
        });
      }
      success = 'Profil mis à jour avec succès.';
    } catch (err) {
      error = err instanceof ApiException ? err.message : 'Erreur lors de la sauvegarde.';
    } finally {
      saving = false;
    }
  }
...
<div class="max-w-2xl mx-auto space-y-6">
  <div>
    ...
    <p class="text-base-content/50 text-sm mt-1">
      Complétez votre profil pour être visible dans l'écosystème BT Capital.
    </p>
  </div>
  ...
  {#if loading}
    <div class="flex justify-center py-8"><LoadingSpinner text="Chargement du profil..." /></div>
  {:else}
    <div class="card bg-base-100 border border-base-200">
      <div class="card-body">
        ...
        <form onsubmit={handleSave} class="flex flex-col gap-4 mt-2">
          ...
          <div class="form-control">
            <label class="label label-text font-medium" for="bio">Biographie</label>
            <textarea
              id="bio"
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="form-control">
              <label class="label label-text font-medium" for="country">Pays</label>
              <input
                id="country"
                type="text"
            ...
            <div class="form-control">
              <label class="label label-text font-medium" for="city">Ville</label>
              <input
                id="city"
                type="text"
                class="input input-bordered w-full"
          ...
          <div class="form-control">
            <label class="label label-text font-medium" for="linkedin">LinkedIn</label>
            <input
              id="linkedin"
              type="url"
              class="input input-bordered w-full"
          ...
          <div class="form-control">
            <label class="label label-text font-medium" for="website">Site web</label>
            <input
              id="website"
              type="url"
              class="input input-bordered w-full"
          ...
          {#if user?.role === 'INVESTOR'}
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="form-control">
                <label class="label label-text font-medium" for="min">Budget min (USD)</label>
                <input
                  id="min"
                  type="number"
                  class="input input-bordered w-full"
              ...
              <div class="form-control">
                <label class="label label-text font-medium" for="max">Budget max (USD)</label>
                <input
                  id="max"
                  type="number"
                  class="input input-bordered w-full"
            ...
            <div class="form-control">
              <span class="label-text font-medium mb-2 block">Secteurs d'intérêt</span>
              <div class="flex flex-wrap gap-2">
                {#each SECTORS as s}
                  <button
                    type="button"
                    class="badge badge-md cursor-pointer transition-colors
                      {preferredSectors.includes(s) ? 'badge-primary' : 'badge-outline'}"
                    onclick={() => (preferredSectors = toggleItem(preferredSectors, s))}
                  >{s}</button>
                {/each}
              </div>
            </div>
            ...
            <div class="form-control">
              <span class="label-text font-medium mb-2 block">Stades d'investissement</span>
              <div class="flex flex-wrap gap-2">
                {#each INVESTMENT_STAGES as s}
                  <button
                    type="button"
                    class="badge badge-md cursor-pointer transition-colors
                      {investmentStages.includes(s) ? 'badge-secondary' : 'badge-outline'}"
                    onclick={() => (investmentStages = toggleItem(investmentStages, s))}
                  >{s}</button>
                {/each}
              </div>
            </div>
          ...
          {#if user?.role === 'ENTREPRENEUR'}
            ...
            <div class="form-control">
              <label class="label label-text font-medium" for="company">Nom de la startup</label>
              <input
                id="company"
                type="text"
                class="input input-bordered w-full"
            ...
            <div class="form-control">
              <label class="label label-text font-medium" for="companyDesc">Description</label>
              <textarea
                id="companyDesc"
                class="textarea textarea-bordered w-full"
                rows="3"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="form-control">
                <label class="label label-text font-medium" for="sector">Secteur</label>
                <select id="sector" class="select select-bordered w-full" bind:value={sector}>
                  <option value="">Choisir...</option>
                  {#each SECTORS as s}<option value={s}>{s}</option>{/each}
                </select>
              </div>
              <div class="form-control">
                <label class="label label-text font-medium" for="stage">Stade</label>
                <select id="stage" class="select select-bordered w-full" bind:value={stage}>
                  <option value="">Choisir...</option>
                  {#each INVESTMENT_STAGES as s}<option value={s}>{s}</option>{/each}
                </select>
              </div>
            </div>
            <div class="form-control">
              <label class="label label-text font-medium" for="funding">Besoin de financement (USD)</label>
              <input
                id="funding"
                type="number"
                class="input input-bordered w-full"
          ...
          {#if user?.role === 'INCUBATOR'}
            ...
            <div class="form-control">
              <label class="label label-text font-medium" for="incName">Nom de l'incubateur</label>
              <input
                id="incName"
                type="text"
                class="input input-bordered w-full"
            ...
            <div class="form-control">
              <label class="label label-text font-medium" for="incDesc">Description</label>
              <textarea
                id="incDesc"
                class="textarea textarea-bordered w-full"
                rows="3"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="form-control">
                <label class="label label-text font-medium" for="duration">Durée du programme</label>
                <input
                  id="duration"
                  type="text"
                  class="input input-bordered w-full"
              ...
              <div class="form-control">
                <label class="label label-text font-medium" for="maxStartups">Capacité</label>
                <input
                  id="maxStartups"
                  type="number"
                  class="input input-bordered w-full"
            ...
            <div class="form-control">
              <span class="label-text font-medium mb-2 block">Secteurs de focus</span>
              <div class="flex flex-wrap gap-2">
                {#each SECTORS as s}
                  <button
                    type="button"
                    class="badge badge-md cursor-pointer transition-colors
                      {focusSectors.includes(s) ? 'badge-accent' : 'badge-outline'}"
